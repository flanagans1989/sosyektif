...
### Son değişiklikler — 2026-09-18 (kapsamlı denetim + düzeltmeler)
- **Denetim bulguları düzeltildi (hepsi `main`'de, `npm run typecheck` + `astro check` + build temiz):**
  - *Çift paylaşım riski:* `kuyruk.ts` `kaydetVeYayinla` artık push sonucunu kontrol ediyor; push başarısızsa kuyruk durur, admin'e uyarı gider, iş kırmızı biter. `gitYayinla.ts` başarısız rebase'i `rebase --abort` ile geri alır.
  - *Hız sınırı devre kesici:* Meta "Application request limit reached" (kod 4/17/32/613) artık deneme hakkı yemez; kanal o çalışmada bırakılır, ≥1 saat sonra tekrar denenir. Kanal başına tur kotası: Instagram 2, Facebook 3 içerik/çalışma (`TUR_SINIRI`). Önceden bu hata içeriği 4 denemede kalıcı `vazgecildi` yapıyordu.
  - *Sağlık denetimi onarım hatası:* vazgeçilen paylaşımları kuyruğa geri alma `dagitim.json`'u yerelde değiştirip hiç push etmiyordu (mekanizma hiç çalışmamıştı) — `saglikDenetimi.ts` artık push ediyor.
  - *Worker sertleştirme (deploy edildi, sürüm 2):* `/tepki` ve `/bildir` için Origin kontrolü + IP başına bellek içi hız sınırı; `/tepki` yalnızca sitede var olan yazı için yazar; sabit zamanlı sır karşılaştırması (`sirEsit`, boş secret asla eşleşmez); `approvePost` yalnızca frontmatter'daki `taslak: true`'yu değiştirir; `resolveSlug` Git Trees API (1000 dosya sınırı yok); `GET /surum` (sağlık denetimi `BEKLENEN_WORKER_SURUMU` ile karşılaştırır — Worker kodu değişince `WORKER_SURUMU` (worker) ve `BEKLENEN_WORKER_SURUMU` (kontroller.ts) birlikte artırılır). Tepki sayaçları ayrı `TEPKI` KV'sinde (`sosyektif_tepki`), eski sayaçlar `METRIKLER`'den okunur.
  - *XSS yüzeyi:* `site/src/lib/html.ts` (`guvenliJson`, `kacis`) — tüm `set:html={JSON…}` ve `burc`/`gunun-sorusu` `innerHTML` şablonları kaçışlı; moderasyon `TEHLIKELI_HTML` (script/iframe/`on…=`/`javascript:`) içeren içeriği sert reddeder. `site/public/_headers` (nosniff, X-Frame-Options, Referrer-Policy, Permissions-Policy; CSP bilerek yok).
  - *Zaman aşımları:* LLM çağrıları 90 sn, canlılık HEAD yoklaması 10 sn.
  - *CI/altyapı:* `pr-kontrol.yml` (agents typecheck + site check/build + worker tsc), `dependabot.yml` (major sürüm atlamaları KAPALI — canlı yayın sistemi), action'lar SHA'ya sabitlendi, bot commit e-postası GitHub noreply.
- **Yeni mekanizmalar:**
  - `data/config.json` → `kapaliKanallar: string[]`: listedeki kanala dağıtım ve sağlık denetimi HİÇ istek atmaz (paylaşımlar `bekliyor`'da kalır). Şu an `["threads"]`.
  - Dağıtım durumu `iptal`: elle temizlenen işler; kuyruk yeniden denemez, sağlık denetimi geri almaz (`vazgecildi`'nin aksine). İptal edilmiş Reels'in eski Telegram ✅ düğmesi paylaşımı tetiklemez (`reel.ts` kapısı).
  - `worker/telegram-onay/scripts/instagram-token-yaz.mjs`: yeni Instagram token'ını sorar (ekranda görünmez), `graph.instagram.com/me` ile doğrular, `meta_token:instagram` KV'sine yazar (`AGENT_PAYLASIM_ANAHTARI` gerekmez, wrangler girişi yeter).
  - **Worker artık `npx wrangler deploy` ile deploy edilebiliyor** (wrangler `deraksizolasyon@gmail.com` hesabına OAuth ile bağlı; Worker bu hesapta). Üretim deploy'u Claude Code izin sistemince engellenir → kullanıcı `! cd worker/telegram-onay && npx wrangler deploy` çalıştırır. `dashboard-paste.js` hâlâ `tsc` çıktısıdır (başlık 8 satır + derleme) ama artık gerekmez.
- **Operasyonel durum (18 Eylül akşamı):** Instagram token'ı yenilendi (şifre değişikliği oturumu geçersiz kılmıştı, `code 190`); ilk denemeler Meta'nın saatlik uygulama sınırına takıldı, devre kesici eklendi. **Threads hesabı Meta incelemesinde** ("itiraz açıldı, ~1 saat, uymazsa kalıcı kapanır") → Threads `kapaliKanallar`'da. Birikmiş işler silindi (8 taslak, Instagram 25 / Reels 24 / Telegram 31 bekleyen → `iptal`). `gunlukHedefIcerikSayisi` 5 → **1**. Yayınlanan içerik: 31 (kapı ~150). Sağlık denetimi: 0 hata (geçici `truncgil` ağ hatası hariç).
- **Bilinen açık noktalar:** `/tepki`+`/bildir` hız sınırı yalnızca Worker örneği başına (kalıcı sınır için Durable Object ya da zone altında özel alan adı + WAF gerekir); Worker sırları hâlâ `?anahtar=` sorgu parametresinde (cron-job.org URL'si + agents istemcileri birlikte değişmeli); git'te ~38 "Dağıtım durumu" commit'i/gün (durum-başına-kanal commit güvenlik tasarımı).
...
- **Yeni kategori — "Ekodektif" (ekonomi):** `kategori` şemasına `ekonomi` eklendi (site + ajanlar, `KATEGORILER`/`kategoriSchema`). `/ekodektif/` sayfası: canlı piyasa panosu + para çevirici (truncgil/CoinGecko, client-side, anahtarsız) ve "ekonomi" kategorisindeki içerikler bir arada. İçerik prompt'una ekonomi odağı yönergesi eklendi: paranın/bankacılığın tarihi, kavramlar, marka hikâyeleri — güncel kur/faiz tahmini ve yatırım tavsiyesi UYGUN DEĞİL. Header/Footer'a link eklendi, kendi rengi (`--color-eko`) ve logosu (`EkodektifLogo.astro`) var. Henüz "ekonomi" kategorisinde yayınlanmış içerik yok — ilk üretim ajan zincirinin bu kategoriyi seçmesini bekliyor.
  - _Aynı gün, 2. tur:_ Sayfaya grafik stüdyosu (Dolar/Euro 2005'ten beri — Frankfurter/ECB; Gram Altın PAXG, Bitcoin/Ethereum — CoinGecko; 7G–2005+ dönemler, üzerine gelince günlük değer), "zaman makinesi" (seçilen yılın Ocak kuru vs bugün, ECB referans kuru), "Biliyor muydun?" notları ve cep sözlüğü eklendi; hero yenilendi. Ana sayfaya kompakt tek satırlık Ekodektif şeridi (`EkodektifBandi.astro`). Fiyat çekme kodu `site/src/lib/piyasa.ts`'te toplandı (PiyasaSeridi, şerit ve sayfa aynı isteği paylaşır). Tümü tarayıcı tarafı, anahtarsız, sıfır maliyet.
...
**✅ ÇÖZÜLDÜ 2026-09-13 (akşam) — dağıtım kuyruğu:** Kullanıcı "her içeriği paylaşmaya başlayalım" dedi; aşağıdaki sorun ve ayrıca gerçek hesaplarda tespit edilen üç sessiz arıza giderildi (Bluesky'de 0 gönderi — 300 grafem sınırı aşılıyordu; Instagram/Facebook her seferinde "görseller push edilemedi" — git çakışması; Facebook süresiz token'ı süresi dolmuş sayılıyordu). Dağıtım artık üretimden ayrı ve duruma dayalı: `agents/src/distribute/kuyruk.ts` + `.github/workflows/dagitim.yml`, durum `data/dagitim.json` (içerik × kanal). Otomatik ve elle onaylanan tüm canlı içerikler paylaşılır; başarısız kanal artan beklemeyle (1-2-3 saat) tekrar denenir, 4 denemede vazgeçilir (kanal düzelince sağlık denetimi geri alır); son 24 saatin içerikleri hemen, eskiler saatte bir; her kanaldan sonra durum push'lanır (çift paylaşım yok). Tetik: pipeline commit'i, Telegram onayı (Worker), saatlik zamanlayıcı, sağlık denetimi. İlk çalışmada İpek Yolu dört kanala, Roma Bluesky/Facebook'a (Threads'te zaten vardı) paylaşıldı. Telegram herkese açık kanal hâlâ yok (kullanıcı kanal açıp `TELEGRAM_PUBLIC_CHANNEL_ID` eklemeli — eklenince eski içerikler de kuyruktan gider).

**Eski kayıt — bilinen sorun (2026-09-12'de tespit edildi, düzeltmesi bilerek ertelendi — henüz yeterli içerik hacmi yok):**
...
- [x] **2026-09-13 (akşam) — tam sağlık denetimi:** Kullanıcı: "gözetim ajanına sistemin tam çalışma sağlığını devredelim, arada bir tarasın, çalışmayan ne varsa düzeltsin — bak ben sormasam bilmiyorduk." `agents/src/orchestrator/saglikDenetimi.ts` + `saglik/kontroller.ts` + `saglik-denetimi.yml` (UTC 0/6/12/18; Worker zamanlayıcısı + GitHub schedule). ~45 kontrol, hepsi dışarıdan gerçek ölçüm: site sayfaları, zamanlayıcı canlılığı (son pipeline <2.5 sa), her workflow'un son çalışması, veri dosyalarının şemaları, onay bekleyip unutulan taslaklar, LLM (Gemini/Groq/Mistral/Cerebras), Pexels/Unsplash, YouTube API, Wikipedia/Google Trends, Ekodektif veri API'leri, Worker, Cloudflare Analytics, Search Console, Telegram bot/sohbet/kanal, Bluesky girişi + gönderi sayısı, Threads/Instagram/Facebook token'ı + hesaptaki gönderi sayısı + token süresi, YouTube OAuth, dağıtım kaydı ↔ hesap tutarlılığı. **Otomatik onarımlar:** başarısız workflow'u bir kez yeniden başlatır (yalnızca çift paylaşım yapmayanlar), zamanlayıcı durmuşsa pipeline'ı tetikler, kanal düzelince vazgeçilmiş paylaşımları kuyruğa geri alır, bekleyen paylaşım varsa kuyruğu dürter, "ardışık başarısız çalışma" kilidi 6 saati geçmiş ve LLM'ler çalışıyorsa kilidi açar. Kod hatalarını kendisi düzeltemez — ne yapılacağını yazar. Bildirim: `data/saglik.json` önceki sonucu tutar; yalnızca yeni/çözülen/onarılan sorunlar Telegram'a gider, UTC 06 çalışması tam rapor. Günlük rapor denetimin özetini ve denetimin kendisi 8 saattir çalışmadıysa uyarıyı gösterir. İlk CI çalışması: 45 kontrol, 0 hata, 4 uyarı (Unsplash yok, Telegram kanalı yok, YouTube OAuth yok, Telegram kanalında bekleyen içerik).
- [x] **Genel audit sonrası eklenenler (aynı gün):** Reels videoları artık git'e commit'lenmiyor — Worker KV'de 3 günlük geçici barındırılıyor (`/gecici-dosya`, `/gecici/<ad>`; ~12MB/video git geçmişini haftalar içinde 1GB'a yaklaştıracaktı; Worker güncel değilse eski yönteme düşer). Denetime eklenenler: Cloudflare'deki Worker'ın repodaki sürümün gerisinde kalması, günlük burç/Tarihte Bugün verisinin bayatlaması (otomatik `burc.yml` yeniden başlatma), "ardışık başarısız" duraklatma kilidinin otomatik açılması. Son denetim: 47 kontrol, 0 hata. Kuyruğun ilk iki çalışmasında 27 paylaşım; Instagram düzeltmesi yeni içerikte doğrulandı.
- [x] **Aynı gün audit düzeltmeleri:** Instagram fotoğraf/carousel container'ı işlenmeden yayınlanıyordu ("Media ID is not available" — kuyruğun ilk çalışmasında görüldü); Threads 500 karakter kırpması linki kesebiliyordu; Telegram kanal hataları yutuluyordu; workflow'lar actions v4→v5 (Node 20); günlük rapor etiketi.

### 11.9 Reels/video ajanı
- [x] **2026-09-13 (akşam) — YouTube Shorts + kuyruk entegrasyonu:** `reelsAktif` **açıldı** ve anlamı değişti: artık carousel'in yerine geçmiyor. Carousel/metin paylaşımları her zaman gider; ayrıca her YENİ içerik (son 24 sa) için Reels videosu kuyruktan Telegram onayına düşer (`kanallar.reel: onayda`). ✅ → `reel-paylas.yml` → Instagram Reels + Facebook video + **YouTube Shorts** (`agents/src/lib/youtube.ts`, resumable upload, başlıkta #Shorts). YouTube OAuth kurulumu kullanıcı adımı: Google Cloud'da OAuth istemcisi (Desktop, izin ekranı "In production") → `npm run youtube-yetkilendir` → `YOUTUBE_OAUTH_CLIENT_ID`/`YOUTUBE_OAUTH_CLIENT_SECRET`/`YOUTUBE_REFRESH_TOKEN` secret'ları. **Google kısıtı:** denetimden geçmemiş projeden yüklenen videolar "özel" kilitlenir; YouTube API Services denetim başvurusu (ücretsiz) onaylanana kadar Shorts herkese açık görünmez — reel-paylas bunu Telegram mesajında açıkça yazar.
